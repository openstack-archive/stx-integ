...
install -D -m644 %{SOURCE1} %{buildroot}%{_datadir}/starlingx/login.defs.cgcs
...
%post
if [ $1 -eq 1 ] ; then
	cp -f %{_datadir}/starlingx/login.defs.cgcs %{_sysconfdir}/login.defs
...
%config(noreplace) %{_unitdir}/clear_shadow_locks.service
%config(noreplace) %{_datadir}/starlingx/login.defs.cgcs
