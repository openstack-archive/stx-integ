...
Source2:        token-server-private-key.pem
Source3:        token-server-certificate.pem
Source4:        htpasswd_testuser_testpassword
BuildRequires: systemd
Requires(post): systemd
Requires(preun): systemd
Requires(postun): systemd

Provides: registry-token-server = %{version}-%{release}
Obsoletes: registry-token-server <= 0.9.1-5
...
%gobuild -o bin/registry-token-server %{import_path}/contrib/token-server
...
# install default certificate for token exchange
install -d -p %{buildroot}%{_sysconfdir}/ssl/private
install -m 400 %{SOURCE2} %{buildroot}%{_sysconfdir}/ssl/private/token-server-private-key.pem
install -m 400 %{SOURCE3} %{buildroot}%{_sysconfdir}/ssl/private/token-server-certificate.pem

# install htpasswd file
# This is going away when we replace the back end with keystone
# currently the credentials are testuser//testpassword
install -d -p %{buildroot}%{_sysconfdir}/%{name}/registry
install -p -m 644 %{SOURCE4} %{buildroot}%{_sysconfdir}/%{name}/registry
...
%{_sysconfdir}/%{name}/registry/htpasswd_testuser_testpassword
%{_sysconfdir}/ssl/private/token-server-private-key.pem
%{_sysconfdir}/ssl/private/token-server-certificate.pem
