...
PLUGIN_HTTP_URL = 'http://localhost:'
...
# A predictable default string value
UNKNOWN = 'unknown'
...
class NetworkObject:
    ...
    def manage_iface_alarm(self):
        """ """
        # Single Link Config
        if self.link_two.name == UNKNOWN:
            if self.link_one.state == DOWN:
        ...
        # link timestamps
        elif self.link_one.state == UP and self.link_two.state == DOWN:
            if self.severity != fm_constants.FM_ALARM_SEVERITY_MAJOR:
                self.timestamp = self.link_two.timestamp
                self.raise_iface_alarm(fm_constants.FM_ALARM_SEVERITY_MAJOR)

        elif self.link_one.state == DOWN and self.link_two.state == UP:
            if self.severity != fm_constants.FM_ALARM_SEVERITY_MAJOR:
                self.timestamp = self.link_one.timestamp
                self.raise_iface_alarm(fm_constants.FM_ALARM_SEVERITY_MAJOR)

        elif self.link_one.state == UP and self.link_two.state == UP:
            if self.severity != fm_constants.FM_ALARM_SEVERITY_CLEAR:
                self.clear_iface_alarm()

        elif self.link_one.state == DOWN and self.link_two.state == DOWN:
            if self.severity != fm_constants.FM_ALARM_SEVERITY_CRITICAL:
...
obj = pc.PluginObject(PLUGIN, PLUGIN_HTTP_URL)


# Network Object Array - Primary Network/Link Control Object
NETWORKS = {NetworkObject(NETWORK_MGMT),
            NetworkObject(NETWORK_OAM),
            NetworkObject(NETWORK_INFRA)}

# Used to index NETWORK Object Array
OBJ_INDEX_NETWORK_MGMT = 0
OBJ_INDEX_NETWORK_OAM = 1
OBJ_INDEX_NETWORK_INFRA = 2
...
def dump_network_info(network):
    ...
    link_two_info = ''
    if network.link_two.name != UNKNOWN:
...
#              for each. Any that are raised are precicely cleared.
#
#              Prevents stuck alarms over port and interface reconfig.
#
#              If the original alarm case still exists thenthe alarm will
...
# Name       : manage_alarm
#
# Purpose    : Raises or clears port and interfface alarms based on
#              calling parameters.
#
# Returns    : True on failure and False on success
#
...
def config_func(config):
            ...
            with open(fn, 'r') as infile:
                for line in infile:
                    if 'lmon_query_port' in line:
                        if isinstance(int(line.split()[2]), int) is True:
    ...
    if url_updated is False:
        ...
        for node in config.children:
            ...
            if key == 'port':
                if isinstance(int(val), int) is True:
...
def read_func():
    ...
    try:
                ...
                for network in NETWORKS:
                    if network.name == network_link_info['network']:
                        ...
                        if len(links) > 0:
                            ...
                            if network.link_one.name == UNKNOWN:
        ...
        for network in NETWORKS:
            if network.link_two.name != UNKNOWN and \
