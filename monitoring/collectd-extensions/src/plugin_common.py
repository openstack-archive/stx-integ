...
class PluginObject(object):
    ...
    def __init__(self, plugin, url):
        ...
        self.fm_connectivity = False     # set true once fm connectivity ok
                                         # set false on fm connectivity failure
    ...
    def is_virtual(self):
        ...
        try:
            cmd = '/usr/bin/facter | grep is_virtual'
            res, err = processutils.execute(cmd, shell=True)
            if err:
                return False
            else:
                # expect 'res' format to be 'is_virtual => true' (or false)
                if len(res.split()) == 3:
                    # ['is_virtual', '=>', 'true']
                    if res.split()[2] == 'true':
