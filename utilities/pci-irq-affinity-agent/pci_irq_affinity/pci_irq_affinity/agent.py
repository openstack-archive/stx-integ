...
from nfv_common.helpers import Object, Constants, Constant, Singleton, coroutine

from config import CONF, sysconfig
...
def get_inst(instance_uuid, callback):
    ...
    nova = NovaProvider()
    instance = nova.get_instance(instance_uuid)
    if instance is not None:
        LOG.info("inst:%s" % instance)
        callback(instance)

def query_instance_callback(instance):
    LOG.info("query inst:%s" % instance)
    pciIrqAffinity.affine_pci_dev_instance(instance)
...
def rpc_message_filter(message, msg_type):
    ...
    LOG.info("Msg filter:msg_type=%s" % event_type)
    if payload is not None:
        LOG.debug(payload)
        server_uuid = payload.get('instance_id', None)

        LOG.info("%s: server_uuid=%s." % (event_type, server_uuid))
...
def start_rabbitmq_client():
    ...
    _rpc_listener = rpc_listener.RPCListener(cfg['rabbit_host'], cfg['rabbit_port'],
                                             ...
                                             cfg['rabbit_virtual_host'], "nova", "notifications.info",
...
@timers.interval_timer('audit_affinity', initial_delay_secs=INITIAL_PCI_IRQ_AUDIT_DELAY, interval_secs=CONF.pci_affine_interval)
def _audit_affinity():
    while True:
        timer_id = (yield)
