...
def start_rabbitmq_client():
    ...
    _rpc_listener = rpc_listener.RPCListener(cfg['rabbit_host'], cfg['rabbit_port'],
                                             ...
                                             "notifications.info",
                                             'notifications.pci_irq_affinity_queue')

    _rpc_listener.add_message_handler(NOVA_SERVER_CREATE,
                                      rpc_message_server_create_filter,
                                      instance_create_handler)

    _rpc_listener.add_message_handler(NOVA_SERVER_DELETE,
                                      rpc_message_server_delete_filter,
                                      instance_delete_handler)

    _rpc_listener.add_message_handler(NOVA_SERVER_RESIZE,
...
def _audit_affinity():
    while True:
        timer_id = (yield)
        LOG.debug("Audit timer_id=%s." % timer_id)
