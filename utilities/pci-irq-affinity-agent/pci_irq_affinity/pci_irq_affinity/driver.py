...
class AffinePciIrqDriver:
    ...
    def affine_pci_dev_irqs(self, instance, wait_for_irqs=True):
        ...
        def _wait_for_msi_irqs(self, instance):
        ...
        if len(instance.get_pci_devices()) == 0:
            return
        ...
        if wait_for_irqs:
            timer = loopingcall.FixedIntervalLoopingCall(
                        _wait_for_msi_irqs, self, instance)
            timer.start(interval=CONF.msi_irq_check_interval).wait()

        @synchronized(instance.uuid)
        def do_affine_pci_dev_instance():
            ...
            inst = NovaProvider().get_instance(instance.uuid)
            if inst is None:
                return

            numa_topology = inst.get_numa_topology()
            extra_spec = inst.get_extra_spec()
        ...
        return do_affine_pci_dev_instance()
