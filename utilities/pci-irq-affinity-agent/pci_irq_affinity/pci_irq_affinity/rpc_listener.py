...
class RPCListener(threading.Thread):
    ...
    def run(self):
        ...
        def _connection_error(exc, interval):
            LOG.error("Connection error, exception=%s" % exc)

        while not self._exit.is_set():
            try:
                if not self._connection.connected:
                    LOG.error("RPC-Listener not connected to exchange %s, queue=%s."
                              % (self._exchange_name, self._consumer_queue_name))
                    self._connection.ensure_connection(errback=_connection_error, interval_start=2,
                                                       interval_step=0, interval_max=2)
                    self._consumer.revive(self._connection)
                    LOG.error("RPC-Listener connected to exchange %s, queue=%s."
                              % (self._exchange_name, self._consumer_queue_name))

                self._consumer.consume()
                self._connection.drain_events(timeout=1)
            except exceptions.TimeoutError as e:
                LOG.debug("Timeout exception, exception=%s" % e)

            except Exception as e:
                LOG.debug("Exception received, exception=%s." % e)
                time.sleep(2)
