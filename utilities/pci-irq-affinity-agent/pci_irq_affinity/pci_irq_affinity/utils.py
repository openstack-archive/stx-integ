...
def get_pci_irqs_pinned_cpuset(extra_spec=None, numa_topology=None,
    ...
    LOG.info("extra_spec:%s, topo:%s, numa_node:%s" % (extra_spec, numa_topology, pci_numa_node))
...
def write_smp_affinity_file(filename, irqs, cpulist):
...
def set_irqs_affinity_by_pci_address(pci_addr, extra_spec=None,
    ...
    LOG.info("pci: %s, irqs: %s, msi_irqs: %s" % (pci_addr, _irqs, _msi_irqs))
    ...
    LOG.info("cpuset where irqs are to be affined:%s or %s" % (cpuset, cpulist))
    ...
    irqs = write_smp_affinity_file('smp_affinity_list', _irqs, cpulist)
    msi_irqs = write_smp_affinity_file('smp_affinity_list', _msi_irqs, cpulist)
    return (irqs, msi_irqs, numa_node, cpulist)
